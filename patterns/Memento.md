...
The memento pattern is a behavioural design pattern that captures and
externalizes an object's internal state without violating encapsulation,
allowing the object to be restored to this state later. This pattern is useful
for implementing undo/redo functionality. It involves creating a memento object
that stores the state of the originator object and providing methods to save and
restore the state. The memento pattern ensures that the object's state can be
rolled back or restored without exposing its internal structure.
